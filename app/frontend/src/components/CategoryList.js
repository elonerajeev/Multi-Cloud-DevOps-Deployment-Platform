...
//     const categoryLoading = new Array(13).fill(Array.map)
...
const CategoryList = () => {
    ...
    const fetchCategoryProduct = async () => {
        ...
        try {
            ...
            const validData = Array.isArray(dataResponse.data)
                ? dataResponse.data.filter(product => product && product.category)
                : [];

            setCategoryProduct(validData);
        } catch (error) {
                        ...
                        categoryProduct.map((product, index) => {
                            return (
                                <Link
                                    ...
                                    <div className='w-16 h-16 md:w-20 md:h-20 rounded-full overflow-hidden p-4 bg-slate-200 flex items-center justify-center'>
                                        <img
                                            src={product.productImage?.[0] || '/default-image.jpg'}
